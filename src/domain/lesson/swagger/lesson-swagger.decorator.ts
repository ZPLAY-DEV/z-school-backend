...
**기능 개요:**
- 과목의 기간(start~end)과 그룹의 요일/시간 정보를 기준으로 수업일을 완전히 재생성합니다
- **기존 수업일은 모두 삭제되고 새로운 일정으로 대체됩니다**
...
5. **기존 데이터 교체**: 기존 수업일 삭제 후 새로운 수업일 생성

**휴일 처리:**
- ✅ 학교 캘린더의 휴일 정보 확인
- ✅ 휴일에 해당하는 날짜는 수업일에서 제외
- ✅ 공휴일, 방학, 시험 기간 등 자동 반영

**데이터 처리:**
- 시간(시작시간+종료시간)이 동일한 기존 수업일만 유지
- 시간이 다른 기존 수업일은 모두 삭제됨
- 새로운 일정에 맞는 수업일 추가 생성
- 각 그룹의 총 수업일 수(days) 자동 업데이트

**사용 시나리오:**
- 과목 기간 변경 후 수업일 재생성
- 그룹 요일/시간 변경 후 일정 업데이트
- 캘린더 업데이트 후 휴일 반영
- 수업 일정 전면 재조정

**⚠️ 중요한 주의사항:**
- **기존 수업일이 삭제될 수 있습니다** (시간 변경 시)
- **관련된 출석 데이터에 영향을 줄 수 있습니다**
- **신중하게 사용해야 하는 기능입니다**
      `,
    }),
    ApiParam({
