...
export class SchoolSamService {
  ...
  async list(schoolId: number): Promise<Sam[]> {
    const queryBuilder = this.samRepository
      ...
      .leftJoinAndSelect('sam.groups', 'groups')
      .where('sam.schoolId = :schoolId', { schoolId })
      .orderBy('sam.id', 'ASC');
  ...
  async infiniteList(
    ...
    try {
      ...
      return await paginate<Sam>(query, queryBuilder, {
        sortableColumns: ['alias', 'id', 'score', 'createdAt'],
        searchableColumns: ['alias', 'instructor.name', 'instructor.phone'],
      ...
      this.logger.error(
        `Failed to fetch sams for school ${schoolId}`,
        error.stack,
      );
      throw error;
    }
